...
var vite_config_default = defineConfig({
  ...
  resolve: {
    alias: {
      "@": path.resolve("/app/server", "client", "src"),
      "@shared": path.resolve("/app/server", "shared"),
      "@assets": path.resolve("/app/server", "attached_assets")
    }
  },
  root: path.resolve("/app/server", "client"),
  build: {
    outDir: path.resolve("/app/server", "dist/public"),
    emptyOutDir: true
  },
  server: {
    fs: {
...
function serveStatic(app2) {
  const distPath = path2.resolve("/app/server", "public");
...
(async () => {
  console.log("\u{1F680} Starting AIForecast Hub (Professional Edition) - DEPLOYMENT FIX v1.0.1");
  console.log("\u{1F527} DEPLOYMENT: Fixed port, caching, and build process");
  console.log(`\u{1F680} Environment: ${process.env.NODE_ENV || "development"}, Port: ${parseInt(process.env.PORT || "5000", 10)}`);
