...
function CommodityChartCard({ commodity, aiModels, onClick }: CommodityChartCardProps) {
  ...
  return (
    <Card 
      className="glass-card hover-lift shimmer cursor-pointer smooth-transition"
      onClick={onClick}
    >
              ...
              <div className="grid grid-cols-1 gap-2">
                {accuracyData && accuracyData.length > 0 ? (
                  accuracyData.slice(0, 3).map(modelData => (
                    <div key={modelData.aiModel.id} className="flex items-center justify-between py-2 px-3 hover:bg-gray-50/50 transition-all duration-200">
                  ...
                  aiModels.slice(0, 3).map(model => (
                    <div key={model.id} className="flex items-center justify-between py-2 px-3 hover:bg-gray-50/50 transition-all duration-200">
                      <div className="flex items-center space-x-2">
                        <div 
                          className="w-2 h-2 rounded-full" 
