...
export class DatabaseStorage implements IStorage {
  ...
  private async testConnection() {
    const maxRetries = 20; // Try for about 2 minutes (exponentialish)
    let retries = 0;
    ...
    while (retries < maxRetries) {
      try {
        ...
        // Special handling for emergency migration if needed
        if (process.env.NODE_ENV === 'production' && retries === 0) {
          // Only try emergency migration ONCE on the first failure
          // If the DB is down (ENOTFOUND), this will fail anyway, which is fine
          // If the DB is up but schema is broken, this might fix it
          console.log("🚨 Attemping emergency checks...");
          try {
            await this.runEmergencyMigration();
            // If successful, continue to next loop iteration which will try normal connection again
        ...
        // If we've run out of retries, throw the error to crash the app
        if (retries >= maxRetries) {
          console.error("💀 Exhausted all database connection retries");
