    ...
    try {
      ...
      for (const commodity of commodities) {
        ...
        if (predictions.length > 0) {
          // Use predictions with future target dates (or recent past for momentum)
          // This is the correct approach: we want to know what AI models predict for the future
          const now = new Date();
          const pastCutoff = new Date(now);
          pastCutoff.setDate(pastCutoff.getDate() - 30); // Include last 30 days for momentum calculations

          const relevantPredictions = predictions.filter(p => {
            const targetDate = new Date(p.targetDate);
            return targetDate >= pastCutoff; // Include recent past + all future predictions
          });

          // If no relevant predictions, use the latest available ones as fallback
          const finalPredictions = relevantPredictions.length > 0
            ? relevantPredictions
