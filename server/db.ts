...
if (!process.env.DATABASE_URL) {
  ...
  // Log the database host being used (masking credentials)
  try {
    const url = new URL(process.env.DATABASE_URL);
    console.log(`🔌 Configuring database connection to host: ${url.hostname}`);
  } catch (e) {
    console.log("🔌 Configuring database connection (URL parsing failed, check format)");
  }
}
